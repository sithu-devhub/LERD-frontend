...
export default function RegionPage() {
  ...
  const [selected, setSelected] = React.useState(() => new Set(["Naimur Rahman", "Shafiq Hammad", "Alex Semuyel"]));
  ...
  const totalCount = regions.length - 1; // excluding header “Region Name”
  ...
  return (
    <div className="p-0">
      <h1 className="text-xl md:text-2xl font-semibold text-[#2B3674] text-center mb-4">Region</h1>
      ...
      {/* Table card with blue border on focus-within */}
      <div
        className="
      ...
      >
        ...
        <div className="max-h-[56vh] overflow-auto">
          {filtered.map((name, idx) => {
            ...
            if (isHeader) return null; // header already shown above

            const checked = selected.has(name);
            const rowHighlight = name === "Naimur Rahman" ? "bg-[#F7FAFF]" : ""; // subtle example from mock
